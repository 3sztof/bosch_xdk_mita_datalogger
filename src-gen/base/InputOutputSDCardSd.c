...
Retcode_T InputOutputSDCardSd_Writer_Write(char** value)
{
	...
	fileOpenReturn = f_open(&fileWriteHandle, "AccelerometerData.csv", FA_WRITE | FA_OPEN_ALWAYS);
	if ((FR_OK == sdCardReturn) && (FR_OK == fileOpenReturn))
	{
		sdCardReturn = f_stat("AccelerometerData.csv", &sdCardFileInfo);
 ...
 */
Retcode_T InputOutputSDCardSd_CycleInit_Write(char** value)
{
	Retcode_T retcode = RETCODE_OK;
	FRESULT sdCardReturn = FR_OK, fileOpenReturn = FR_OK;
	FIL fileWriteHandle;
 ...
 */
Retcode_T InputOutputSDCardSd_CycleWriter_Write(char** value)
{
	Retcode_T retcode = RETCODE_OK;
	FRESULT sdCardReturn = FR_OK, fileOpenReturn = FR_OK;
	FIL fileWriteHandle;
	...
	sdCardFileInfo.lfname = NULL;
	#endif
	uint16_t cycleWriterFilePosition;
	fileOpenReturn = f_open(&fileWriteHandle, ".cycle.xdk", FA_WRITE | FA_OPEN_ALWAYS);
	if ((FR_OK == sdCardReturn) && (FR_OK == fileOpenReturn))
	{
		sdCardReturn = f_stat(".cycle.xdk", &sdCardFileInfo);
		cycleWriterFilePosition = sdCardFileInfo.fsize;
	}
	if ((FR_OK == sdCardReturn) && (FR_OK == fileOpenReturn))
	{
	    sdCardReturn = f_lseek(&fileWriteHandle, cycleWriterFilePosition);
	}
	if ((FR_OK == sdCardReturn) && (FR_OK == fileOpenReturn))
	{
	    sdCardReturn = f_write(&fileWriteHandle, *value, strlen(*value), &bytesWritten); /* Write it to the destination file */
...
Retcode_T InputOutputSDCardSd_CycleReader_Read(char** result)
{
	...
	if ((FR_OK == sdCardReturn) && (FR_OK == fileOpenReturn))
	{
	    sdCardReturn = f_read(&fileReadHandle, *result, 1, &bytesRead); /* Read a chunk of source file */
	}
	if ((FR_OK == sdCardReturn) && (FR_OK == fileOpenReturn))
	{
	}
 ...
 */
Retcode_T InputOutputSDCardSd_CycleReader_Write(char** value)
{
	BCDS_UNUSED(value);
	return RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_FAILURE);
